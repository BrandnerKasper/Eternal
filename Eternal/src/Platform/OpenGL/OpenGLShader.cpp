...
namespace Eternal {
	...
	static GLenum ShaderTypeFromString(const std::string& type)
	{
		...
		ET_CORE_ASSERT(false, "Unkown shader type!");
		return 0;
	}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			ET_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specification!");
	...
	{
		/*The idea of this Code was abstracted from:
		...
		*/
		...
		for (auto& keyValue : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				ET_CORE_ASSERT(false, "Shader compilation failed!");
				break;
			}
		...
		if (isLinked == GL_FALSE)
		{
			...
			ET_CORE_ERROR("{0}", infoLog.data());
			ET_CORE_ASSERT(false, "Linking shader failed!");
			return;
		}
