...
namespace Eternal {
	...
	void Eternal::OpenGLVertexArray::AddVertexBuffer(SharedPtr<VertexBuffer>& vertexBuffer)
	{
		glBindVertexArray(m_RendererID);
		vertexBuffer->Bind();
	...
	void Eternal::OpenGLVertexArray::SetIndexBuffer(SharedPtr<IndexBuffer>& indexBuffer)
	{
		glBindVertexArray(m_RendererID);
		indexBuffer->Bind();
